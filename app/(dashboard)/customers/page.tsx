...
export default function CustomersPage() {
  ...
  const filtered = customers.filter((c) => {
    ...
    const matchesState = stateFilter === "all" || c.state_code === stateFilter;
    ...
    },
    {
      ...
      render: (row) =>
        row.gstin ? (
          ...
          <span className="text-xs text-neutral-400 italic">Unregistered (B2C)</span>
        ),
    },
    {
      ...
      render: (row) => (
        <span
          className={`font-mono font-bold text-xs ${
            row.balance > 0 ? "text-income-700" : "text-neutral-700"
          }`}
        >
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-neutral-500">
            Client directory, verified GSTIN identification, and ledger balances.
          </p>
        </div>
      ...
      <div className="p-4 rounded-xl border border-neutral-200 bg-white shadow-card flex flex-col sm:flex-row gap-3 items-center justify-between">
        <div className="w-full sm:w-80">
          <Input
            placeholder="Search by name, GSTIN, PAN or phone..."
            value={search}
      ...
      <DataTable
        ...
        emptyDescription={
          search || stateFilter !== "all"
            ...
            : "Get started by adding your first registered or unregistered customer."
        }
        emptyAction={
          search || stateFilter !== "all"
            ? undefined
